...
A numbered, growing list of **predictors**, run on the staged change before any push. One clean split settles *where* they run: `release-update` **stages** — it applies the change to the template (the first consumer), producing the exact state that will ship and proving the document applies cleanly; the `validation` skill's **pre-ship phase** then **runs** these tests against that staged state, scoping the suite to the pillars the change touches and executing the applicable ones as the `dry-run-validation` workflow — one dedicated agent per test (a real cold-start walkthrough, an adversarial red-team, a persona role-play, a theory critique), not a checklist — before any push. A `fail` blocks the push; a `flag` is the maintainer's call (ship-with-risk is allowed). New entries arrive from the calibration loop (§2) — each carries the field failure that motivated it, and is added to both this table and the workflow's test set.
